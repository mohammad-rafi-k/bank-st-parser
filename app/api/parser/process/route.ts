...
import { genAI } from '@/app/lib/utils/geminiClient';
...
export async function POST(request: Request) {
    try {
        const { fileUrl, fileType, fileBuffer } = await request.json();
        ...
        const verificationStatus = deepseekResult
            ? JSON.stringify(geminiParsedData) === JSON.stringify(deepseekResult)
                ? 'verified'
                : 'mismatch'
            : 'pending';
